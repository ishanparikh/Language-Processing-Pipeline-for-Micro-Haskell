...
class MH_Typechecker {
	...
	throws TypeError, UnknownVariable {
        ...
        else if (exp.isVAR()) return env.typeOf(exp.value());
        else if (exp.isAPP())
        {
            ...
            if (fst.isArrow())
            {
                if ((fst.left()).equals(snd))
                    ...
                    throw new TypeError("The type of the second expression doesn't match the type expected");
            }
            else
                throw new TypeError("The expression doesn't contain arrows");
        }
        else if (exp.isINFIX()) {
            MH_TYPE fst = computeType(exp.first(), env);
            MH_TYPE snd = computeType(exp.second(), env);
            if(fst.isInteger() && snd.isInteger())
            {
                if (exp.infixOp().equals("==") || exp.infixOp().equals("<=")) return BoolType;
                else if (exp.infixOp().equals("+") || exp.infixOp().equals("-")) return IntegerType;
                else throw new TypeError("The operator is not defined");
            }
            else throw new TypeError("The arguments for the infix operator are not of the right type");
        }
        else if(exp.isIF()){
            MH_TYPE fst = computeType(exp.first(), env);
            MH_TYPE snd = computeType(exp.second(), env);
            MH_TYPE thd = computeType(exp.third(), env);
            if(fst.isBool())
            {
                if(snd.equals(thd))
                    return thd;
                else
                    throw new TypeError("The types of e2 and e3 in if \"e1 then e2 else e3\" are different");
            }
            else
                throw new TypeError("The type of e1 in if \"e1 then e2 else e3\" is not boolean");
        }
        else
